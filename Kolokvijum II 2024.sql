...
DELETE * 
FROM RADNO_MESTO
WHERE RADNO_MESTO.ZJMBG IN (SELECT ZAPOSLENI.JMBG
  FROM RADNO_MESTO.ZJMBG INNER JOIN ZAPOSLENI ON RADNO_MESTO.ZJMBG = ZAPOSLENI.JMBG
  INNER JOIN KOMPANIJA ON RADNO_MESTO.IDK = KOMPANIJA.IDK
  WHERE KOMPANIJA.GODINA_OSN < 2000 
  GROUP BY ZAPOSLENI.JMBG
  HAVING COUNT(CASE WHEN RADNO_MESTO.DATUMDO IS NULL THEN RADNO_MESTO.DATUMDO - RADNO_MESTO.DATUMOD END) > 1000
